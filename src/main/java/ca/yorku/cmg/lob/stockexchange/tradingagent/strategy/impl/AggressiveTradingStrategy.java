...
public class AggressiveTradingStrategy implements ITradingStrategy {
    ...
    public AggressiveTradingStrategy(TradingAgent tradingAgent) {
    ...
    public void actOnEvent(Event e, int pos, int price) {
        ...
        if (e instanceof GoodNews) {
            newOrder = new Bid(tradingAgent.getTrader(), e.getSecrity(),(int) Math.round(price*1.05), (int) Math.round(pos*0.5),e.getTime());
        } else if (e instanceof BadNews) {
            newOrder = new Ask(tradingAgent.getTrader(), e.getSecrity(),(int) Math.round(price*0.90), (int) Math.round(pos*0.8),e.getTime());
        } else {
